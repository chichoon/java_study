    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mHandler = new Handler(){
            public void handleMessage(Message msg){
                for(int i = 0; i < 10; i++){
                    Balls.get(i).x += Balls.get(i).Speed * Balls.get(i).DirectionX;
                    Balls.get(i).y += Balls.get(i).Speed * Balls.get(i).DirectionY;
                    if(Balls.get(i).x >= 800){
                        Balls.get(i).DirectionX = -1;
                    }
                    else if(Balls.get(i).x < 0){
                        Balls.get(i).DirectionX = 1;
                    }
                    if(Balls.get(i).y >= 800){
                        Balls.get(i).DirectionY = -1;
                    }
                    else if(Balls.get(i).y < 0){
                        Balls.get(i).DirectionY = 1;
                    }
                }
                vw.invalidate();
